...
let catalog = null;
    ...
    function loadCurrentDuelStateIfMatching(fighterId, opponentId) {
      ...
      try {
        ...
        if (state.fighterId !== fighterId || state.opponentId !== opponentId) {
          return false;
        }
    ...
    function updateBodyDisplays() {
      document.getElementById("myBodyDisplay").textContent = myCurrentBody + " / " + myMaxBody;
      document.getElementById("opponentBodyDisplay").textContent = opponentCurrentBody + " / " + opponentMaxBody;
      
      document.getElementById("fixedMyBody").textContent = myCurrentBody + " / " + myMaxBody;
      document.getElementById("fixedOpponentBody").textContent = opponentCurrentBody + " / " + opponentMaxBody;
      ...
      if (opponentCurrentBody <= -5) {
        ...
        status.innerHTML = '<span class="success">Adversaire sous 1 Point de Corps : combat terminé.</span>';
        ...
        status.innerHTML = '<span class="danger">Tu es sous 1 Point de Corps : hors combat.</span>';
      } else {
        status.textContent = "";
      }
    }
    ...
    function getRestrictionInfo(restriction) {
      switch (restriction) {
        ...
        case "only_brown":
          return { label: "Seulement Marron / Distance Accrue", css: "restriction-brown" };
...
function updateRestrictionBanner(restriction) {
  ...
  banner.textContent = info.label;
    ...
    function calculateTemporaryBonus(page, action) {
      ...
      switch (bonusMode) {
        ...
        case "score_plunge_or_lateral":
          return (
            label.includes("coup plongeant") ||
            label.includes("coup latéral")
          ) ? 2 : 0;
    ...
    function resolveTurn() {
      ...
      if (resultPageNumber === undefined || resultPageNumber === null) {
        alert(
          ...
          "Ton PG : " + myMovementPage + "\n" +
          "PG reçu : " + enemyMovementPage
        );
        return;
      }
      ...
      if (!page) {
        alert("La page résultat " + resultPageNumber + " existe dans la table, mais pas dans la liste des pages.");
        return;
      }
      ...
      if (page.score === null || page.score === undefined) {
        ...
        damageHtml =
          ...
          'SCORE ' + page.score +
          ' + MOD ' + selectedAction.mod +
          ' + bonus ' + (Number(selectedAction.bonus || 0) + calculateTemporaryBonus(page, selectedAction));
      ...
      const imagePath =
        page.image ||
        "images/" + currentBook.id + "/LW_" + currentBook.id + "_" + resultPageNumber + ".png";

     const imageSrc = imagePath + '?v=' + Date.now();

    imageHtml =
      '<div class="page-image-box image-priority-box">' +
      '<img class="page-image priority-image" src="' + imageSrc + '" alt="Page ' + resultPageNumber + '" onclick="openImageOverlay(this.src)" onerror="this.parentElement.style.display=\'none\'">' +
      '<button type="button" class="image-zoom-button" onclick="openImageOverlay(\'' + imageSrc + '\')">Agrandir l’image</button>' +
      '</div>';
      ...
      document.getElementById("resultText").innerHTML =
        '<div class="result-card">' +
          ...
          '<div class="result-meta">' +
            '<span><strong>Action</strong> : ' + actionLabel(selectedAction) + '</span>' +
            '<span><strong>PG</strong> : ' + selectedAction.pg + '</span>' +
            '<span><strong>Reçu</strong> : ' + enemyPg + '</span>' +
          '</div>' +
    ...
    function applyDamageToOpponent() {
      ...
      if (lastDamage === 0) {
        status.innerHTML = '<span class="success">Aucun dégât. Points de Corps adverses inchangés.</span>';
      } else {
        status.innerHTML =
          '<span class="success">' +
          lastDamage +
